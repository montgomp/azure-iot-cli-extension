...
import unittest
import mock
from azext_iot.product.command_test import initialize_workspace
...
class InitClass(unittest.TestCase):
    ...
    def test_init_with_no_working_folder(self, mock_dump, mock_open, mock_mkdir, mock_join, mock_exists):
    ...
    @mock.patch('builtins.open', create=True)
    @mock.patch('json.dump')
    def test_init_with_working_folder(self, mock_dump, mock_open, mock_mkdir, mock_join, mock_exists):
        mock_exists.return_value = False
        mock_join.return_value = 'filepath'
